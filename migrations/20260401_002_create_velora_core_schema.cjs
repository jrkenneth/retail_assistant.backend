...
exports.up = async function up(knex) {
  ...
  await knex.schema.createTable("customers", (table) => {
    ...
    table.enu("account_status", ["active", "suspended", "closed"], {
      useNative: true,
  ...
  await knex.schema.createTable("products", (table) => {
    ...
    table.enu("availability_status", ["in_stock", "low_stock", "out_of_stock"], {
      useNative: true,
  ...
  await knex.schema.createTable("orders", (table) => {
    ...
    table.enu("delivery_status", ["processing", "in_transit", "out_for_delivery", "delivered", "failed"], {
      useNative: true,
  ...
  await knex.schema.createTable("returns", (table) => {
    ...
    table.uuid("order_id").notNullable().references("id").inTable("orders").onDelete("CASCADE");
    table.uuid("customer_id").notNullable().references("id").inTable("customers").onDelete("CASCADE");
    table.enu("status", ["requested", "approved", "rejected", "completed"], {
      useNative: true,
      enumName: "return_status",
    }).notNullable();
    table.text("reason").notNullable();
    table.decimal("refund_amount", 12, 2).nullable();
    table.enu("refund_status", ["pending", "processed", "not_applicable"], {
      useNative: true,
  ...
  await knex.schema.createTable("support_tickets", (table) => {
    ...
    table.text("description").notNullable();
    table.enu("status", ["open", "in_progress", "escalated", "resolved", "closed"], {
      useNative: true,
  ...
  await knex.schema.createTable("loyalty_transactions", (table) => {
    ...
    table.uuid("customer_id").notNullable().references("id").inTable("customers").onDelete("CASCADE");
    table.uuid("order_id").nullable().references("id").inTable("orders").onDelete("SET NULL");
    table.enu("transaction_type", ["earned", "redeemed", "adjusted", "expired"], {
      useNative: true,
  ...
  await knex.schema.createTable("audit_log", (table) => {
    ...
    table.enu("event_type", ["access_denied", "escalation_triggered", "refusal_triggered", "scope_violation"], {
      useNative: true,
      enumName: "velora_audit_event_type",
    }).notNullable();
    table.string("domain", 120).notNullable();
    table.string("intent", 120).notNullable();
    table.jsonb("params_snapshot").notNullable().defaultTo("{}");
    table.text("reason").notNullable();
  ...
  await knex.raw("create index if not exists idx_chat_sessions_customer_started on chat_sessions (customer_id, started_at desc)");
  await knex.raw("create index if not exists idx_chat_messages_session_created on chat_messages (session_id, created_at)");
  await knex.raw("create index if not exists idx_audit_log_customer_created on audit_log (customer_id, created_at desc)");
};
